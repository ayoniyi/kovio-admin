...
const Withdrawals = () => {
  ...
  const withdrawalsQuery: any = useQuery({
    queryKey: ["withdrawals", searchQuery.length >= 3 ? searchQuery : ""],
    queryFn: () => {
      const endpoint =
        searchQuery.length >= 3
          ? `/wallet/admin/withdrawals?page=1&limit=10&search=${encodeURIComponent(searchQuery)}`
          : `/wallet/admin/withdrawals?page=1&limit=10`;
  ...
  if (withdrawalsQuery?.isLoading && !withdrawalsData && !searchQuery) {
    return <Loader />;
  }
              ...
              <TableRow key={withdrawal?._id}>
                ...
                <TableCell>
                  <div
                    className="relative"
                    ref={
                      openDropdownId === withdrawal?._id ? dropdownRef : null
                    }
                  >
                    <svg
